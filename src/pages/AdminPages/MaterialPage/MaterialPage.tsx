...
const MaterialPage: React.FC = () => {
  ...
  const fetchDeletedStats = async () => {
    try {
      const deletedResponse = await materialApi.getDeleted({
        page: 1,
        limit: 100,
      });
            ...
            <table className="min-w-full bg-white rounded-md overflow-hidden border">
              ...
              <tbody>
                {displayMaterials.map((material) => (
                  <tr key={material._id} className="hover:bg-gray-50 border-t">
                    <td className="py-2 px-4 border-b font-mono text-xs">
                      {material._id.slice(-8)}
                    </td>
                        ...
                        {!showDeleted ? (
                          ...
                          <>
                            ...
                            {canUpdate() && (
                              <button
                                onClick={() =>
                                  handleRestoreMaterial(material._id)
                                }
