...
const AddProduct: React.FC<AddProductProps> = ({ handleClose }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        setError("Không th? t?i d? li?u. Vui lòng th? l?i!");
      } finally {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!formData.name.trim()) {
      setError("Tên s?n ph?m không du?c d? tr?ng!");
      setLoading(false);
      return;
    }
    if (formData.name.trim().length < 2 || formData.name.trim().length > 200) {
      setError("Tên s?n ph?m ph?i có t? 2-200 ký t?!");
      setLoading(false);
      return;
    }
    if (!formData.description.trim()) {
      setError("Mô t? s?n ph?m không du?c d? tr?ng!");
      setLoading(false);
      return;
    }
    if (
      ...
      setError("Mô t? s?n ph?m ph?i có t? 10-1000 ký t?!");
      setLoading(false);
      return;
    }
    if (!formData.category) {
      setError("Vui lòng ch?n danh m?c!");
      setLoading(false);
      return;
    }
    if (!formData.brand) {
      setError("Vui lòng ch?n thuong hi?u!");
      setLoading(false);
      return;
    }
    ...
    try {
      ...
      const errorMessage =
        error.response?.data?.message || "Thêm s?n ph?m th?t b?i!";
        ...
        <form className="space-y-4" onSubmit={handleSubmit}>
          <div className="mb-4">
            <label className="block text-sm font-medium text-mono-700 mb-1">
              Tên S?n Ph?m <span className="text-mono-800">*</span>
            </label>
            <input
              type="text"
              name="name"
          ...
          <div className="mb-4">
            <label className="block text-sm font-medium text-mono-700 mb-1">
              Mô T? <span className="text-mono-800">*</span>
            </label>
            <div className="relative">
              <textarea
                ...
                required
                rows={5}
                placeholder="Mô t? chi ti?t v? s?n ph?m: ch?t li?u, d?c di?m n?i b?t, công d?ng..."
            ...
            <p className="mt-1 text-xs text-mono-500">
              ?? Mô t? chi ti?t giúp khách hàng hi?u rõ hon v? s?n ph?m
            </p>
          </div>
            ...
            <div className="border-2 border-mono-300 rounded-lg p-4 bg-mono-50 max-h-64 overflow-y-auto">
              {loadingData ? (
                <div className="flex items-center justify-center py-8">
                  ...
                  <span className="ml-3 text-mono-600">Ðang t?i tags...</span>
                </div>
              ) : tags.length === 0 ? (
            ...
            {formData.tags.length > 0 && (
              <div className="mt-2 flex items-center gap-2">
                <span className="text-sm font-medium text-mono-700">
                  Ðã ch?n: {formData.tags.length} tag(s)
                </span>
                <button
                  type="button"
